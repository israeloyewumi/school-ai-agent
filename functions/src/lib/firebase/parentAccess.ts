// functions/src/lib/firebase/parentAccess.ts - ADMIN SDK VERSION
...
// Import other Admin SDK functions we need
import { getStudentById } from './studentManagement';
import { getStudentAttendance, getStudentMerits, getStudentResultsRaw } from './db';
import { getStudentFeeStatus as getFeeStatusFromManagement, getStudentPayments } from './feeManagement';
// ===== INTERFACES =====
      ...
      .map(m => {
        ...
        try {
          if (m.date instanceof Date) {
            ...
            awardDate = (m.date as any).toDate();
          } else {
...
export async function getStudentFeeStatus(
  ...
  try {
    ...
    // Get calculated fee status from feeManagement
    const feeStatusData = await getFeeStatusFromManagement(studentId, term, session);
    ...
    const paymentHistory: FeePayment[] = payments.map(payment => {
      ...
      try {
        if (payment.paymentDate instanceof Date) {
          ...
          paymentDate = (payment.paymentDate as any).toDate();
        } else if (payment.recordedAt && typeof payment.recordedAt === 'object' && 'toDate' in payment.recordedAt) {
          paymentDate = (payment.recordedAt as any).toDate();
        } else {
      ...
      return {
        id: payment.id,                        // @ts-ignore
        amount: payment.amountPaid || payment.amount || 0,
        date: paymentDate,                       // @ts-ignore
        method: payment.paymentMethod || payment.method || 'Unknown',
        receiptNumber: payment.receiptNumber || '',  // @ts-ignore
        recordedBy: payment.receivedByName || payment.recordedBy || 'System'
      };
    });

    // If fee status was calculated successfully
    if (feeStatusData) {
      console.log('✅ Using calculated fee status from feeManagement');
    ...
    // Fallback if fee status couldn't be calculated
    console.log('⚠️ Could not calculate fee status, showing payments only');
...
/**
 * Get detailed attendance records with date conversion
 */
...
/**
 * Get detailed merit records with date conversion
 */
...
/**
 * Get detailed grade records with date conversion
 */
    ...
    return records.map(record => {
      ...
      try {
        if (record.dateRecorded instanceof Date) {
          ...
          recordDate = (record.dateRecorded as any).toDate();
        } else {
