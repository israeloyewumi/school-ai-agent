// functions/src/lib/firebase/classManagement.ts - ADMIN SDK VERSION
...
import { ALL_CLASSES } from '../config/schoolData';
...
export async function initializeAllClasses(): Promise<void> {
  try {
    ...
    for (const classInfo of ALL_CLASSES) {
      ...
      const classRef = db.collection('classes').doc(classInfo.classId);
...
/**
 * ✅ FIXED: Check if class has a valid class teacher
 */
...
/**
 * ✅ FIXED: Get available classes (no class teacher assigned)
 */
...
/**
 * ✅ REWRITTEN: Assign subject teacher to a class
 */
