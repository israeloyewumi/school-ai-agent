// functions/src/lib/firebase/dailyRecordings.ts - ADMIN SDK VERSION
// ✅ COMPLETE VERSION WITH STUDENT NAME LOOKUP
...
// Import helper functions from other Admin SDK files
import { getTeacher } from './db';
import { getStudentById } from './studentManagement';  // ✅ Use the correct function name
// ============================================
// INTERFACES
// ============================================
...
export async function getAllTeachersDailyRecordings(
  ...
  const startOfDay = admin.firestore.Timestamp.fromDate(getStartOfDay(date));
  const endOfDay = admin.firestore.Timestamp.fromDate(getEndOfDay(date));
  ...
  try {
    ...
    const meritsSnapshot = await db.collection('merits')
      .where('date', '>=', startOfDay)
      .where('date', '<=', endOfDay)
      .where('term', '==', term)
      .where('session', '==', session)
    ...
    const gradesSnapshot = await db.collection('results')
...
/**
 * NEW: Simplified function for admin chat agent - Get daily teacher activity
 * This replaces the client SDK logic in adminChatAgent.ts
 */
export async function getDailyTeacherActivity(
  ...
  // Get all teachers
  const { getAllTeachers } = await import('./db');
